...
<body>
                ...
                <div class="dashboard_content_main">
                    <header>
                        <div class="header-box">
                            <h1>DEPARTMENTS</h1>
                            <button id="openModalButton">ADD NEW DEPARTMENT</button>
                        </div>
                    </header>
                            ...
                            <tbody id="departmentTableBody">
                                @foreach($departments as $department)
                                    <tr>
                                        ...
                                        <td>
                                            <div class="status-box {{ $department->status === 'Active' ? 'status-active' : 'status-inactive' }}">
                                                {{ $department->status }}
                                            </div>
                                        </td>
                                        <td>
                                            <button onclick="editDepartment('{{ $department->id }}', '{{ addslashes($department->name) }}', '{{ $department->status }}')" class="action-button">
                                                <i class="fas fa-pencil-alt action-icon"></i> 
                                            </button>
                                            <button onclick="deleteDepartment('{{ $department->id }}')" class="action-button"></button>
   ...
   <script src="{{ asset('js/depart.js') }}"></script>
